 ...
 * httpServletRequest.getHeader(AUTHORIZATION) : ""; token=
 * StringUtils.removeStart(token, "Bearer").trim(); Authentication
 * requestAuthentication = new UsernamePasswordAuthenticationToken(token,
 * token); return
 * getAuthenticationManager().authenticate(requestAuthentication);
 * 
 * }
 * 
 * @Override protected void successfulAuthentication(final HttpServletRequest
 * request, final HttpServletResponse response, final FilterChain chain, final
